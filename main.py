...
def student_view_subject_analysis(subject, type):
    if request.method == 'GET':
        ...
        if type=="subtopic" :
            ...
            return render_template('student_view_subject_analysis.html', user=user, result=result, subject=subject, type=type)
        elif type=="test" :
            ...
            for res in response:
                if res['test_id'] not in result.keys():
                    ...
                    result[res['test_id']][0] += res['marks_score']
                    result[res['test_id']][1] += res['marks_total'] 
            return render_template('student_view_subject_analysis.html',test=test, type=type, subject=subject, result=result)
